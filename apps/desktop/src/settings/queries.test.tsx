...
describe("SQLite settings", () => {
  ...
  it.each([
    ...
    "moves the legacy Corola %s selection to its on-device provider",
...
Start with decisions.`);
  ...
  it("updates against the latest SQLite value inside the write queue", async () => {
    mocks.execute.mockResolvedValue([
      {
        ...
        value_json: JSON.stringify(JSON.stringify(["Corola"])),
      },
    ]);
    ...
    expect(next).toBe(JSON.stringify(["Corola", "Erebor"]));
    const statement = mocks.executeTransaction.mock.calls[0][0][0];
    expect(statement.params.slice(0, 2)).toEqual([
      "personalization_dictionary_terms",
      JSON.stringify(next),
