...
const DEV_BUNDLE_ID: &str = "com.minuteswise.dev";
const LEGACY_STABLE_BUNDLE_ID: &str = "com.hyprnote.Hyprnote";
#[cfg(any(target_os = "macos", target_os = "linux"))]
const MANAGED_CLI_DIR: &str = ".minuteswise-cli";
const STABLE_BUNDLE_ID: &str = "com.minuteswise.desktop";
...
pub fn check<R: tauri::Runtime, T: tauri::Manager<R>>(manager: &T) -> EmbeddedCliStatus {
    ...
    let Some(install_path) = install_path_for_command(command_name) else {
        ...
        let missing_dir = "MinutesWise could not find your local application data directory.";
        #[cfg(not(target_os = "windows"))]
        let missing_dir = "MinutesWise could not find your home directory.";
    ...
    #[cfg(target_os = "windows")]
    {
        let Some(_resource_path) = resolve_resource_path(manager) else {
            return EmbeddedCliStatus {
                ...
                details: Some("The CLI is not included in this build of MinutesWise.".to_string()),
            };
        };
    ...
    #[cfg(target_os = "windows")]
    {
        match status.state {
            ...
            EmbeddedCliState::Conflict => {
                return Err(format!(
                    "Another file already exists at {}. Move it before installing the MinutesWise CLI.",
                    status.install_path
                ));
            }
...
fn command_name_from_identifier(identifier: &str) -> &'static str {
    match identifier {
        STAGING_BUNDLE_ID => "minuteswise-staging",
        DEV_BUNDLE_ID => "minuteswise-dev",
        STABLE_BUNDLE_ID | LEGACY_STABLE_BUNDLE_ID => "minuteswise",
        _ => "minuteswise",
    }
}
...
fn bundled_binary_name() -> Option<&'static str> {
    ...
    {
        return Some("minuteswise-cli-x86_64-pc-windows-msvc.exe");
    }

    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    {
        return Some("minuteswise-cli-aarch64-apple-darwin");
    }

    #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
    {
        return Some("minuteswise-cli-x86_64-apple-darwin");
    }

    #[cfg(all(target_os = "linux", target_arch = "x86_64"))]
    {
        return Some("minuteswise-cli-x86_64-unknown-linux-gnu");
    }

    #[cfg(all(target_os = "linux", target_arch = "aarch64"))]
    {
        return Some("minuteswise-cli-aarch64-unknown-linux-gnu");
    }
...
fn details_for_state(state: EmbeddedCliState, install_path: &Path) -> Option<String> {
    match state {
        EmbeddedCliState::Installed => Some(format!(
            "Installed at {} and managed by MinutesWise.",
            install_path.display()
        )),
        EmbeddedCliState::Missing => Some(format!(
