...
Use MinutesWise's typed, read-only interfaces for meeting data. Do not use `find`,
`grep`, `rg`, filesystem crawling, or direct SQLite queries to find or read
meetings.

Prefer the MinutesWise MCP tools when they are available:
...
If MCP is unavailable, use the MinutesWise CLI with `--json`:

```sh
minuteswise --json meetings list --query "planning"
minuteswise --json meetings get MEETING_ID
minuteswise --json meetings transcript MEETING_ID --limit 200 --offset 0
minuteswise --json meetings history MEETING_ID
```

The CLI discovers MinutesWise's database from the platform application-data
...
Install the bundled CLI and agent skill from **MinutesWise → Settings → Developers**.
