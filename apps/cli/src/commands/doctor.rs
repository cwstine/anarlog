...
async fn schema_check(db: &anlg_db_core::Db) -> std::result::Result<(), String> {
    tokio::try_join!(
        anlg_db_app::get_session(db.pool(), "__minuteswise_doctor__"),
        anlg_db_app::list_session_documents(db.pool(), "__minuteswise_doctor__"),
        anlg_db_app::list_session_transcripts(db.pool(), "__minuteswise_doctor__"),
        anlg_db_app::list_session_participants(db.pool(), "__minuteswise_doctor__"),
        anlg_db_app::list_session_action_items(db.pool(), "__minuteswise_doctor__"),
...
fn render(report: &DoctorReport) -> String {
    ...
    let mut lines = vec![
        format!("MinutesWise CLI {}", report.cli_version),
