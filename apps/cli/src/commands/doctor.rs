...
async fn schema_check(db: &anlg_db_core::Db) -> std::result::Result<(), String> {
    tokio::try_join!(
        anlg_db_app::get_session(db.pool(), "__corola_doctor__"),
        anlg_db_app::list_session_documents(db.pool(), "__corola_doctor__"),
        anlg_db_app::list_session_transcripts(db.pool(), "__corola_doctor__"),
        anlg_db_app::list_session_participants(db.pool(), "__corola_doctor__"),
        anlg_db_app::list_session_action_items(db.pool(), "__corola_doctor__"),
...
fn render(report: &DoctorReport) -> String {
    ...
    let mut lines = vec![
        format!("Corola CLI {}", report.cli_version),
