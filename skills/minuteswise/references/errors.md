...
Run `minuteswise --json doctor`. Ask the user to open MinutesWise once if the database does not exist. If they keep data in a custom location, use `--db-path FILE` or `MINUTESWISE_DB_PATH` after they provide the path.
...
Run `minuteswise --json meetings list` to distinguish database access from client configuration. Confirm the MCP command is `minuteswise` and its only required argument is `mcp`. Missing meetings and proposals are invalid parameters. Validation and conflict failures, including declining a non-pending proposal, are internal MCP errors.
