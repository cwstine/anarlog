---
name: minuteswise
description: Query MinutesWise meetings, notes, summaries, transcripts, participants, action items, and recurring history. Use when a user asks about their MinutesWise meeting data or needs meeting context for another task.
---

# MinutesWise

Use the local `minuteswise` CLI or local MCP server. Meeting reads are safe. Writes
...
2. Otherwise use local `minuteswise --json meetings ...` commands.
...
Never query or modify MinutesWise's SQLite database directly. The CLI and MCP servers handle application-schema compatibility.
...
- To stage an edit, use the local CLI or local MCP proposal tools. A human
  applies or declines it in the MinutesWise desktop app.
