---
title: "CLI reference"
description: "Commands and global options available in the MinutesWise CLI."
---

## Global syntax

```text
minuteswise [--base DIR] [--db-path FILE] [--json] <command>
```

| Option | Environment variable | Behavior |
| --- | --- | --- |
| `--base DIR` | `MINUTESWISE_BASE` | Use `DIR/app.db`. |
| `--db-path FILE` | `MINUTESWISE_DB_PATH` | Use an explicit SQLite file. |
| `--json` | — | Emit a versioned JSON response or machine-readable JSON error. |

`--db-path` takes precedence over `--base`. Without either option, the CLI uses MinutesWise's platform application-data directory and recognized legacy locations.

On Flatpak, the host command is `minuteswise-cli`. On DEB, AppImage, macOS, Windows, and Settings-installed builds, the command is `minuteswise`. Substitute `minuteswise-cli` only when that is the command on your PATH.

All commands operate on the local database. The CLI has no account, login, or
hosted meeting source.
...
```bash
minuteswise --help
minuteswise meetings --help
minuteswise meetings list --help
minuteswise --version
```
