---
title: "CLI reference"
description: "Commands and global options available in the Corola CLI."
---

## Global syntax

```text
corola [--base DIR] [--db-path FILE] [--json] <command>
```

| Option | Environment variable | Behavior |
| --- | --- | --- |
| `--base DIR` | `COROLA_BASE` | Use `DIR/app.db`. |
| `--db-path FILE` | `COROLA_DB_PATH` | Use an explicit SQLite file. |
| `--json` | — | Emit a versioned JSON response or machine-readable JSON error. |

`--db-path` takes precedence over `--base`. Without either option, the CLI uses Corola's platform application-data directory and recognized legacy locations.

On Flatpak, the host command is `corola-cli`. On DEB, AppImage, macOS, Windows, and Settings-installed builds, the command is `corola`. Substitute `corola-cli` only when that is the command on your PATH.
...
```bash
corola --help
corola meetings --help
corola meetings list --help
corola --version
```
